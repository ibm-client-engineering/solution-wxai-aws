...
echo "******** Start remove bootstrap from load balancer"
...
aws elbv2 deregister-targets --target-group-arn ${int_api} --targets Id=${bs_ip}
aws elbv2 deregister-targets --target-group-arn ${int_svc} --targets Id=${bs_ip}

echo "******** Finished removing bootstrap from load balancer"
